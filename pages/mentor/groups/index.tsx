...
export default function MentorGroups() {
  ...
  const title = useMemo(() => t('Groups'), [t])
  return (
    <>
      <Head><title>{t('Groups')} - Mentor</title></Head>
      <div className="max-w-7xl mx-auto p-6">
        <BackButton href="/dashboard" className="mb-4" />
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">{title}</h1>
          <motion.button whileTap={{ scale: .98 }} onClick={()=>setOpen(true)} className="px-4 py-2 rounded bg-primary-600 text-white">{t('Create Group')}</motion.button>
        </div>
